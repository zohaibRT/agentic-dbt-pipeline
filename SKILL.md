...
- Target **50+** supported rows in `measure_catalog.md` and **30+** in `metric_catalog.md` when gold evidence allows; do not stop at three to five executive KPIs. Read [reporting-coverage-requirements.md](references/reporting-coverage-requirements.md). Run `python <skill>/scripts/check_analytics_coverage.py --root <project.root>` before presentation — **FAIL** if catalogs are thin while gold has 3+ facts/marts. Expand every `fct_` / `mart_` into counts, amounts, status mixes, quality flags, and partner/program/product/date slices; keep `kpi_catalog` as the smaller strategic subset.
