...
7. Mandatory repeated section: **Still blocked — fix these or these KPIs stay missing** — list OPEN Attention Board IDs, the missing/confusing data or definition, and the makeable KPIs that remain unavailable until the human fixes them. Include this even when the same warning was shown at prior checkpoints. If none, write `No open KPI gaps at this checkpoint.`
8. Trusted now vs still-blocked KPI lists (from `KPI_GAP_REGISTER.md` / approved contracts).
9. Recommended next action and exactly what approval would permit — and that next Yes does **not** unlock blocked KPIs unless matching OPEN IDs are answered.

Read [kpi-gap-and-stakeholder-warnings.md](references/kpi-gap-and-stakeholder-warnings.md) and [stakeholder-layer-and-presentation-guide.md](references/stakeholder-layer-and-presentation-guide.md) before writing the checkpoint chat summary.
