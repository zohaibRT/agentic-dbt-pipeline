...
If `.env` is missing, create it safely from `.env.example`, list available dbt profiles from `~/.dbt/profiles.yml` with profile name, adapter, and non-secret notes, stop before discovery or dbt commands, and ask me for DBT_DOMAIN, DBT_PROFILE_NAME, and DBT_SOURCE_SCHEMA. Do not search the repository, inspect terminal output, infer, suggest, or summarize values from other workspaces or previous runs. Do not choose a profile automatically.
