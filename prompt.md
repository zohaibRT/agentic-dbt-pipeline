...
For Power BI PBIP/TMDL artifacts, run a validation loop before handoff: static file checks, relationship ambiguity audit, Power BI Modeling Model Context Protocol `ConnectFolder` and model inspection when available, DAX smoke query, and Power BI Desktop open validation when available. Static checks must include root `.pbip` shortcut schema validation where report deliverables use the required `report` property and not unsupported `dataset`, `.platform` `$schema` values matching the supported Fabric git integration platform properties pattern, `report.json` `themeCollection.baseTheme.reportVersionAtImport` presence/type, no table with more than one `IsKey = True` column, and TMDL table files with no invalid loose Power Query `let`/`in` lines outside valid expression blocks. Fix and repeat until validation passes. Do not report the presentation layer as complete unless `presentation_report.md` and `PIPELINE_STATUS.md` show PASS with evidence, or clearly show BLOCKED/NOT RUN for unavailable required checks.
