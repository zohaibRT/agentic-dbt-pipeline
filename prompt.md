...
If `.env` is missing, create it safely from `.env.example`, stop before discovery or dbt commands, and ask me to fill the required non-secret values. Do not create `.env` from sibling project patterns. Do not use sibling projects or previous runs as the active source/profile unless I confirm them.
