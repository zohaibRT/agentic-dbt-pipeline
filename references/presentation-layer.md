...
6. For approved Matplotlib work, also write or update `reports/agent/10_presentation/matplotlib/README.md`, `requirements-matplotlib.txt`, `report_spec.md`, `kpi_figure_coverage.md`, `label_dictionary.md`, `report_theme.md`, `report_theme.py`, `serve_report.py`, `data_access.py`, optional `generate_report.py`, `report_builder.py`, `report_pages/`, `report.html`, `open_report.bat`, optional `figures/`, and `sql_verification/` per [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md). Treat the local web report served by `serve_report.py` plus `report.html` as the primary deliverable and figures as optional exports. Use the comfortable colorful theme, business-friendly labels, full key performance indicator coverage, refresh behavior, and SQL verification required by that reference.
