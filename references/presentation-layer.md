...
|---|---|---|
| **Matplotlib** | Python-based refreshable web report with SQL-verified charts generated from validated warehouse data | **Recommended default** |
| **Power BI** | Interactive PBIP/TMDL or Desktop-connected semantic model and report handoff | Optional alternative |

Default artifact: if the user approves a presentation layer and does not specify another tool or artifact type, use the **Matplotlib refreshable web report** workflow by default. Read [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md), build `serve_report.py` plus a polished browser-viewable `report.html` with live SQL-verified Matplotlib SVG/HTML or approved browser-native chart routes from validated data, and write artifacts under `reports/agent/10_presentation/matplotlib/`.
...
```text
Analytics insight reporting is complete. Validated key performance indicators: <short KPI/value summary>. A Matplotlib refreshable web report is recommended by default because it is reproducible, version-controlled, browser-viewable, colorful, and can refresh values through SQL-backed Python routes. Power BI is available when you need an interactive Desktop semantic model and report handoff. Recommended page set: <page list>. <Important caveat if any>.
...
- `Yes - build Matplotlib refreshable web report (recommended)` - approves the separate `presentation_layer` phase using the Matplotlib refreshable web report workflow by default.
...
6. For approved Matplotlib work, also write or update `reports/agent/10_presentation/matplotlib/README.md`, `requirements-matplotlib.txt`, `report_spec.md`, `kpi_figure_coverage.md`, `label_dictionary.md`, `report_theme.md`, `report_theme.py`, `serve_report.py`, `data_access.py`, optional `generate_report.py`, `report_builder.py`, `report_pages/`, `report.html`, readiness-gated `open_report.bat` / `open_report.sh`, optional `figures/`, and `sql_verification/` per [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md). Treat the local web report served by `serve_report.py` plus `report.html` as the primary deliverable and figures as optional exports. Use the comfortable colorful theme, business-friendly labels, full key performance indicator coverage, refresh behavior, and SQL verification required by that reference. Do not release open instructions until `REPORT_HANDOFF_READINESS.json` has `open_allowed=true`.
...
| Matplotlib refreshable web report | **Default when the user says yes to presentation layer and does not specify another technology** | Local browser report served by `serve_report.py`, tabbed `report.html`, live SQL-verified Matplotlib SVG/HTML or approved browser-native charts, Python scripts, theme, launcher, SQL verification, and report spec from validated gold/marts data |
...
|---|---|
| Presentation layer, with no technology specified | Matplotlib refreshable web report by default |
...
If the user approves the default presentation layer, the agent must build the Matplotlib refreshable web report under `reports/agent/10_presentation/matplotlib/` per [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md). Do not mark the phase complete with only advice; the phase is pending until the local server smoke test, actual localhost page response validation, chart refresh behavior, and SQL verification are recorded or the work is explicitly blocked.
...
Recommended option: <Matplotlib refreshable web report / Power BI PBIP/TMDL project / dbt documentation only / presentation layer report / dashboard design / semantic layer first / export or query handoff>
...
Recommended default: Matplotlib refreshable web report. It is reproducible, version-controlled, browser-viewable, colorful, and refreshes values through SQL-backed Python routes.
...
- `Yes - build Matplotlib refreshable web report (recommended)`
...
- For approved Matplotlib work, create `reports/agent/10_presentation/matplotlib/` artifacts per [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md), build the local web report served by `serve_report.py` plus `report.html` as the primary deliverable, map all recommended measures and key performance indicators into `kpi_figure_coverage.md`, install missing `matplotlib`, `numpy`, and `pandas` prerequisites when needed, validate that the local report URL returns HTTP 200 and non-empty HTML with `scripts/validate_local_web_report.py`, and record SQL verification for every rendered chart aggregate.
