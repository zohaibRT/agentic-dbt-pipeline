...
Before final delivery, read references/advanced-data-engineering-review.md, references/evidence-driven-dbt-process.md, and references/independent-verification-governance.md. Run `python scripts/run_acceptance_gate.py --root .`, `python scripts/check_requirement_traceability.py --root .`, `python scripts/check_layer_proof_coverage.py --root .`, and `python scripts/verify_metric_reconciliation.py --root .`. Report the senior data-engineering review status. Do not claim delivery complete when any acceptance or verification gate returns FAIL.
