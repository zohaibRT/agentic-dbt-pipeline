...
For a new/full pipeline, run read-only discovery first, write reports/agent/discovery_report.md, update reports/agent/PIPELINE_STATUS.md and reports/agent/CONTEXT_TREE.md, explain what you conclude from the source schemas/tables, and ask whether the user wants to add requirements before planning Bootstrap & Init. If the dbt project root does not exist yet, create reports/agent/ under the current workspace/run root.
