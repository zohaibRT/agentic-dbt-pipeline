...
If the confirmed database, dataset, catalog, or source schema is missing, empty, or appears wrong, stop after metadata-only candidate listing. Recommend the likely replacement with evidence, then wait for user approval before updating `.env`, changing profile settings, profiling candidate tables, writing discovery reports, or continuing discovery.
...
- Do not switch to another database, dataset, catalog, or schema without user approval, even when the configured source is empty and a better candidate is visible
