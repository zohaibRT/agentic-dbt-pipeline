...
For a new/full pipeline, run lightweight read-only project discovery first, write reports/agent/00_discovery/discovery_report.md, reports/agent/00_discovery/requirements.md, reports/agent/00_discovery/core_profile.json, reports/agent/00_discovery/discovery_raw.json, reports/agent/00_discovery/cardinality_report.md, reports/agent/00_discovery/relationship_profile.md, and reports/agent/00_discovery/sql_proofs/, update reports/agent/PIPELINE_STATUS.md and reports/agent/CONTEXT_TREE.md, create necessary Mermaid discovery diagrams including an entity relationship diagram when credible relationships exist, explain what you conclude from the source schemas/tables, include recommended medallion direction for sources, bronze/staging, silver/intermediate, and gold/marts, extract source-derived requirements with evidence/confidence/open questions into requirements.md, link discovery_raw.json queries_executed[] to SQL proof files, recommend the best next path with evidence, and ask whether the user wants to add or change requirements before automatic project setup and configuration. Before asking the discovery approval question, send a normal Markdown chat summary titled `Discovery Complete` with status, source reviewed, key findings, validation or SQL proof highlights, reports written, open decisions, recommended next step, what setup includes, what setup does not include, and how to approve. Do not put discovery findings only inside a native/clickable question. Keep the native question short: `Do you approve this discovery scope and want automatic project setup to run next?` with options `Yes, continue to setup`, `Add requirements first`, and `Tell me what to change`. If the dbt project root does not exist yet, create reports/agent/ under the current workspace/run root and put discovery files under reports/agent/00_discovery/. Do not design every layer upfront; read references/phased-discovery.md and references/recommendation-and-review.md, then run focused discovery before sources, bronze, silver, gold, semantic, evaluator, documentation, and analytics insight reporting phases.
...
Before each non-setup phase that changes files or builds warehouse objects, read references/phase-plan-approval.md, references/recommendation-and-review.md, and references/data-engineer-decision-gate.md, update AGENT_PLAN.md, explain the plan in Markdown with the agent recommendation, explicit data-engineering decisions, evidence, confidence, risks, and approval needs, and wait for approval. Use a native interactive approval question when the runtime supports it; otherwise use the text fallback. Natural approval responses such as Yes, Proceed, Approved, Continue, Run this prompt, Looks good, or Go ahead approve only the displayed active checkpoint plan or next-phase prompt. Do not require exact phrases such as approve sources, approve bronze, approve silver, or approve gold after the exact prompt has been shown. If the user changes scope, models, key performance indicators, privacy, schemas, validation, materialization, or files, revise AGENT_PLAN.md and reports/agent/NEXT_PHASE_PROMPT.md before proceeding. After each bronze/staging, silver/intermediate, or gold/marts build, read references/layer-data-validation.md, run warehouse validation queries for row counts, expected emptiness, grain, relationships, row-count movement, date coverage, status/category distributions, measures, mapping coverage, and privacy exposure, save every validation query as a reusable SQL proof file under the phase `sql_proofs/` folder with captured results, write the results and proof links into the phase report, and share the important validation results with the user before moving to the next phase. After each completed phase, read references/phase-completion-report.md, references/next-phase-prompt.md, and references/context-tree.md, write/update reports/agent/<phase>_report.md, reports/agent/PIPELINE_STATUS.md, reports/agent/CONTEXT_TREE.md, and reports/agent/NEXT_PHASE_PROMPT.md, send a normal assistant message with a visible Markdown chat control-panel summary of what completed and what is next, paste the exact next-phase prompt in that chat message, explain how to approve, and only then ask whether to run it as written using an interactive approval question when available. Keep the interactive question short. Do not show only a clickable/native question, and do not put the whole summary only inside the question body.
Before gold/marts, semantic layer, analytics insight reporting, presentation layer, and final delivery, read references/evidence-driven-dbt-process.md, references/universal-analytics-framework.md, references/kpi-definitions.md, references/kpi-definition-contract.md, references/kpi-discovery-framework.md, references/metric-verification.md, and references/metric-verification-checklist.md. The agent is allowed to build only what it can prove; `dbt build` is necessary but not sufficient. Define key performance indicators with business meaning, source model, grain, numerator, denominator, filters, time field, dimensions, caveats, validation evidence, approval status, and expected-versus-actual reconciliation. Build business process, fact, dimension, measure, and metric catalogs before promoting strategic key performance indicators. Maintain `reports/agent/KPI_DEFINITION_CONTRACTS.md` and `reports/agent/METRIC_VERIFICATION_MATRIX.md`. Do not implement semantic metrics or presentation calculations from ambiguous or unreconciled key performance indicators. Stop before semantic layer, analytics insight reporting, presentation layer, or final delivery when numerator, denominator, filters, or final result do not reconcile from gold SQL to semantic or presentation measures.
After marts, semantic layer, evaluator, and documentation are complete, read references/analytics-insight-reporting.md and references/universal-analytics-framework.md, write/update AGENT_PLAN.md, get approval, and produce analytics insight reporting deliverables under reports/agent/. Include business_process_catalog.md, fact_catalog.md, dimension_catalog.md, measure_catalog.md, metric_catalog.md, kpi_discovery_matrix.md, kpi_catalog.md, reporting_catalog.md, dashboard_spec.md, readiness scorecard, and backlog where applicable. Do not create presentation artifacts in this phase.
After analytics insight reporting is complete, read references/presentation-layer.md, references/matplotlib-presentation-layer.md, references/universal-analytics-framework.md, and references/reporting-standards.md, use dashboard_spec.md, business_process_catalog.md, fact_catalog.md, dimension_catalog.md, kpi_catalog.md, reporting_catalog.md, insight_backlog.md, reporting_readiness_scorecard.md, and analytics_insight_report.md as the scope contract, recommend a Matplotlib refreshable web report as the default presentation technology and Power BI as the interactive alternative, then ask whether the user wants a presentation artifact and which technology to use. Offer `Yes - build Matplotlib refreshable web report (recommended)`, `Yes - prepare Power BI Desktop template handoff`, `No presentation layer - complete final delivery now`, and `Tell me what to change first`. Do not mark final delivery complete until the user declines or approves the presentation artifact path. The presentation layer must not invent pages, key performance indicators, visuals, or business scope that bypass analytics insight reporting unless the user explicitly overrides it. If the user approves without naming a technology, default to Matplotlib and build a rich local browser report with `serve_report.py`, `report.html`, colorful business tabs, live Matplotlib SVG/HTML or approved browser-native charts from refreshed JSON, executive cards, chart cards, insight captions, exception callouts, detail sections, caveats, validation status, refresh control, and launcher under reports/agent/10_presentation/matplotlib/. PNG files are optional exports only, not the primary web rendering path. Map every recommended measure, metric, and key performance indicator from measure_catalog.md, metric_catalog.md, kpi_discovery_matrix.md, and kpi_catalog.md into kpi_figure_coverage.md. Install missing matplotlib, numpy, and pandas prerequisites when needed. If the user explicitly chooses Power BI, run the Power BI handoff workflow. If the user approves, run a separate presentation_layer phase plan, build the approved artifact, validate it, and write the presentation report. For Power BI PBIP/TMDL, actively check whether Power BI Modeling Model Context Protocol tools are available or installable before handoff. If available, use ConnectFolder, model inspection, relationship inspection, and DAX smoke testing. If unavailable, record the tool search, install attempt or recommendation, and exact reason; do not claim Model Context Protocol validation passed or skip it silently.
Before final delivery, read references/advanced-data-engineering-review.md, references/evidence-driven-dbt-process.md, references/independent-verification-governance.md, and references/final-delivery.md. Run `python <installed-skill-path>/scripts/run_acceptance_gate.py --root <project.root>`, `python <installed-skill-path>/scripts/check_requirement_traceability.py --root <project.root>`, `python <installed-skill-path>/scripts/check_layer_proof_coverage.py --root <project.root>`, and `python <installed-skill-path>/scripts/verify_metric_reconciliation.py --root <project.root>`. Report the senior data-engineering review status. Do not claim delivery complete when any acceptance or verification gate returns FAIL. After any requested task, phase, fix, documentation update, presentation artifact, commit, push, or verification run is complete, print a complete summary in the chat pane with status, what was done, validation, files changed or reports written, important notes, and next action. Do not end with only a file path, commit hash, clickable approval widget, "done", or "see report."
...
- After every bronze/staging, silver/intermediate, and gold/marts build, run warehouse data validation queries, write reusable SQL proof files with captured results, write Data Verification Results and SQL Proof Files in the phase report, and share those results with the user before continuing
...
| `analytics_insight_reporting` | [analytics-insight-reporting.md](analytics-insight-reporting.md), [universal-analytics-framework.md](universal-analytics-framework.md) |
| `presentation_layer` | [presentation-layer.md](presentation-layer.md), [matplotlib-presentation-layer.md](matplotlib-presentation-layer.md), [evidence-driven-dbt-process.md](evidence-driven-dbt-process.md), [universal-analytics-framework.md](universal-analytics-framework.md), [reporting-standards.md](reporting-standards.md) |
