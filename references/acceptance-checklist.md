...
- [ ] After analytics with 3+ gold facts/marts, `check_analytics_coverage.py` passes with **50+** measures and **30+** metrics (or documents why the target is impossible)
...
- [ ] If Matplotlib presentation artifacts were created, `python <skill>/scripts/check_presentation_coverage.py --root <project.root>` passed or only warned; catalog rows map in `kpi_figure_coverage.md`, categorical axes use `label_dictionary.md`, every `RENDERED` chart/board item has executed live SQL proof with captured results in `sql_verification/` plus `_proof_index.md`, and the live HTML includes visible **All Measures** (≥50) and **All Metrics** (≥50) boards when gold has 3+ facts per [reporting-coverage-requirements.md](reporting-coverage-requirements.md)
