...
- [ ] When privacy minimization opt-out is recorded, `HUMAN_ATTENTION_BOARD.md` and `KPI_GAP_REGISTER.md` have no OPEN privacy-minimization blockers and `check_privacy_opt_out.py` passes (domain-neutral; no industry field hardcodes)
- [ ] After analytics, process/fact coverage checkers pass (`analytics_coverage_matrix.md`, fact contracts, KPI contracts) — fixed measure/metric counts are advisory only when configured
...
- [ ] If Matplotlib presentation artifacts were created, `python <skill>/scripts/check_presentation_coverage.py --root <project.root>` passed or only warned; catalog rows map in `kpi_figure_coverage.md`, categorical axes use `label_dictionary.md`, every `RENDERED` chart/board item has executed live SQL proof with captured results in `sql_verification/` plus `_proof_index.md`, the live HTML includes visible **All Measures** (≥50) and **All Metrics** (≥50) boards with **Display name** + **formatted values** (not snake_case SQL ids / raw floats), and a **Dimensions** tab shows readable dim tables when gold dims exist per [reporting-coverage-requirements.md](reporting-coverage-requirements.md) Rules 5b–5c
