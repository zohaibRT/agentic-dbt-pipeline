...
- [ ] If Matplotlib presentation artifacts were created, `python <skill>/scripts/check_presentation_coverage.py --root <project.root>` passed or only warned; catalog rows map in `kpi_figure_coverage.md`, categorical axes use `label_dictionary.md`, every `RENDERED` chart has executed live SQL proof with captured results in `sql_verification/`, and the live HTML includes visible **All Measures** (≥50) and **All Metrics** (≥30) boards when gold has 3+ facts per [reporting-coverage-requirements.md](reporting-coverage-requirements.md)
