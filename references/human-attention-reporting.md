...
| No | Exact row counts already proven PASS, template boilerplate, repeated inventory lists, software versions unless blocked, **OPEN privacy-minimization blockers when user opted out** |
...
| G-02 | Privacy-safe entity dim policy? | Enables slicing | **If no opt-out:** hash keys; exclude clear identifiers from gold. **If opt-out recorded:** BUILD dim with business labels; CARRY_FORWARD always-exclude classes only | Identifiers present in source | Clear-text gold without opt-out | `05_gold` proof | Complete star / KPIs | OPEN only when opt-out **not** recorded |
